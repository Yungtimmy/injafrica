...
export default function PlayerProfilePage() {
  ...
  const exactScores = predictions.filter((p) => p.pointsEarned === 6 || p.pointsEarned === 8).length;
  const correctOutcomes = predictions.filter((p) => p.pointsEarned !== null && p.pointsEarned > 0).length;
      ...
      <div className="sb-card overflow-hidden">
        ...
        {predictions.length === 0 ? (
          ...
          predictions.map((pred) => {
            ...
            const ptColor = pts === 6 || pts === 8 ? 'text-sb-yellow' : pts && pts > 0 ? 'text-green-400' : 'text-red-400';
            return (
              <div key={pred._id} className="flex flex-wrap items-center px-4 py-3 border-b border-sb-border hover:bg-sb-card-2 gap-x-3 gap-y-1">
                ...
                <div className="text-center shrink-0">
                  <div className="text-[10px] text-sb-muted">Pick</div>
